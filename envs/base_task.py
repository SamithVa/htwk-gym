    ...
    def render(self):
        ...
        if self.cfg["viewer"]["record_video"]:
            ...
            if self.camera is None:
                ...
                self.camera_frames = []
            if self.root_states.dim() == 2:
                tracked_pos = self.root_states[self.cfg["viewer"]["record_env_idx"], 0:3]
            else:
                tracked_pos = self.root_states[self.cfg["viewer"]["record_env_idx"], 0, 0:3]
            cam_pos = gymapi.Vec3(
                *(x + y for x, y in zip(tracked_pos.tolist(), self.cfg["viewer"]["pos"]))
            )
            cam_target = gymapi.Vec3(*tracked_pos.tolist())
