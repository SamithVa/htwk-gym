...
class KickingRobust(BaseKicking):
    ...
    def _reset_ball_at_robot_front(self, env_ids_to_reset_ball):
        ...
        ball_local_offset = torch.zeros((len(env_ids_to_reset_ball), 3), dtype=torch.float, device=self.device)
        ball_local_offset[:, 0] = apply_randomization(ball_local_offset[:, 0], self.cfg["randomization"].get("ball_init_pos_x"))
        ball_local_offset[:, 1] = apply_randomization(ball_local_offset[:, 1], self.cfg["randomization"].get("ball_init_pos_y"))
    ...
    def _reward_kicking_foot_approach_ball_stationary(self):
        ...
        kicking_foot_index = self.cfg["rewards"].get("kicking_foot_index", 0)
        kicking_foot_pos = self.feet_pos[:, kicking_foot_index, :]
        foot_ball_dist = torch.norm(kicking_foot_pos - current_ball_pos_world, dim=-1)
