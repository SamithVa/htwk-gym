...
class ParameterWalkKick(BaseTask):
    ...
    def _init_buffers(self):
        ...
        # Two actors per env: index 0 = robot, index 1 = ball
        ...
        # Body states include robot bodies + 1 ball body
        ...
        # Ankle DoF indices — used by ankle stability reward terms
        self.ankle_dof_indices = torch.tensor(
            [i for i, name in enumerate(self.dof_names) if "Ankle" in name],
            dtype=torch.long,
            device=self.device,
        )

        # Ball tracking buffers
    ...
    def _get_reward_float(self, key, default):
        value = self.cfg["rewards"].get(key, default)
        return float(value)
    ...
    def _reset_dofs(self, env_ids):
        ...
        # Robot is actor 0 of 2 actors per env; multiply env index by 2 to get robot actor index
    ...
    def _reset_root_states(self, env_ids):
        ...
        # Reset ball in front of the (newly reset) robot; GPU sync done below with robot+ball together
        self._reset_ball_at_robot_front(env_ids, sync_gpu=False)

        # Update both robot and ball actors in the simulation
    ...
    def _reset_ball_at_robot_front(self, env_ids_to_reset_ball, sync_gpu=True):
        ...
        forward_vec_local = torch.tensor([1.0, 0.0, 0.0], device=self.device).unsqueeze(0).repeat(n, 1)
        right_vec_local   = torch.tensor([0.0, 1.0, 0.0], device=self.device).unsqueeze(0).repeat(n, 1)
        forward_vec_world = quat_rotate(robot_quat, forward_vec_local)
        right_vec_world   = quat_rotate(robot_quat, right_vec_local)

        zeros = torch.zeros(n, device=self.device)
        fwd_dist   = apply_randomization(zeros, self.cfg["randomization"].get("ball_init_pos_x"))
        lat_offset = apply_randomization(zeros, self.cfg["randomization"].get("ball_init_pos_y"))

        ball_target_xy = (
            robot_pos[:, 0:2]
            + fwd_dist.unsqueeze(-1) * forward_vec_world[:, 0:2]
            + lat_offset.unsqueeze(-1) * right_vec_world[:, 0:2]
        )
        ...
        if sync_gpu and len(ball_actor_indices) > 0:
            self.gym.set_actor_root_state_tensor_indexed(
                self.sim,
                gymtorch.unwrap_tensor(self.root_states),
    ...
    def step(self, actions):
        ...
        # Ball-only resets (ball out of range but robot still alive)
    ...
    def _check_termination(self):
        ...
        # Ball-based terminations
        max_ball_still_time = self._get_reward_float("max_ball_still_time_s", 4.0)
        self.reset_buf |= self.time_since_ball_is_still_buf > max_ball_still_time
        max_ball_moving_time = self._get_reward_float("max_ball_moving_time_s", 4.0)
    ...
    def _compute_observations(self):
        ...
        # Ball position in robot frame is appended after the 10 command dimensions.
        ...
        ball_obs = apply_randomization(relative_ball_pos[:, 0:2], self.cfg["noise"].get("ball_pos")) * self.cfg["normalization"]["ball_pos"]
    ...
    def _reward_ankle_vel(self):
        # Penalize high ankle joint velocities to suppress erratic ankle motion
        return torch.sum(torch.square(self.dof_vel[:, self.ankle_dof_indices]), dim=-1)

    def _reward_ankle_acc(self):
        # Penalize rapid ankle velocity changes (jerk), reducing impulsive ankle forces
        ankle_acc = (self.last_dof_vel[:, self.ankle_dof_indices] - self.dof_vel[:, self.ankle_dof_indices]) / self.dt
        return torch.sum(torch.square(ankle_acc), dim=-1)
    ...
    def _reward_ball_velocity_target_direction(self):
        ...
        target_position = to_torch(
            self.cfg["rewards"].get("ball_target_position", [5.0, 0.0, 0.05]),
            device=self.device,
        )
        if target_position.shape[-1] == 2:
            target_position = torch.cat([target_position, to_torch([self.ball_radius], device=self.device)], dim=-1)
    ...
    def _reward_kicking_foot_approach_ball_stationary(self):
        """Rewards moving a foot towards the ball via proximity."""
    ...
    def _reward_body_alignment_for_kick(self):
        """Rewards aligning the robot's forward direction with the kick target."""
        ...
        kick_target_pos_world = to_torch(
            self.cfg["rewards"].get("kick_target_pos_world", [5.0, 0.0, self.ball_radius]),
            device=self.device,
        ).unsqueeze(0)
    ...
    def _reward_body_angle(self):
        """Rewards keeping the robot's body relatively flat."""
        base_roll, base_pitch, _ = get_euler_xyz(self.base_quat)
        pitch_normalized = (base_pitch + torch.pi) % (2 * torch.pi) - torch.pi
        roll_normalized = (base_roll + torch.pi) % (2 * torch.pi) - torch.pi

        penalty = torch.square(pitch_normalized) + torch.square(roll_normalized)
        reward = 1.0 / (0.1 + penalty ** 2) - 1.0
        return reward

    def _reward_ball_acceleration(self):
        """Rewards ball acceleration in +x direction (towards target), penalizes lateral acceleration."""
    ...
    def _reward_waiting(self):
        """Quadratic time-in-episode reward — penalizes long episodes when scale is negative."""
        max_ball_still_time = self._get_reward_float("max_ball_still_time_s", 2.0)
        progress = self.episode_length_buf.float() / np.ceil(max_ball_still_time / self.dt)
        return progress * progress

    # ------------ orbit / ring rewards (Stage 2) ----------------
    def _reward_stay_in_ring(self):
        """Quadratic penalty on body-to-ball horizontal distance exceeding `orbit_max_radius`."""
        max_r = self.cfg["rewards"].get("orbit_max_radius", 0.6)
        dist = torch.norm(self.ball_pos[:, 0:2] - self.base_pos[:, 0:2], dim=-1)
        excess = torch.clamp(dist - max_r, min=0.0)
        return torch.square(excess)

    def _reward_orbit_ball(self):
        """Reward tangential (circumferential) motion around the ball.

        Strictly gated to fire only when the robot is already inside `orbit_max_radius`,
        so approach and orbit are cleanly phased.
        """
        ball_to_robot = self.base_pos[:, 0:2] - self.ball_pos[:, 0:2]
        dist = torch.norm(ball_to_robot, dim=-1, keepdim=True).clamp(min=1e-3)
        radial_dir = ball_to_robot / dist
        tangent_dir = torch.stack([-radial_dir[:, 1], radial_dir[:, 0]], dim=-1)  # 90° CCW in xy
        base_vel_xy = self.root_states[:, 0, 7:9]
        tangential_speed = torch.sum(base_vel_xy * tangent_dir, dim=-1)
        max_r = self.cfg["rewards"].get("orbit_max_radius", 0.6)
        inside_ring = (dist.squeeze(-1) < max_r).float()
        return torch.abs(tangential_speed) * inside_ring

    def _reward_approach_ball(self):
        """Reward radial velocity toward the ball while OUTSIDE `orbit_max_radius`.

        Produces a clean approach-phase signal that shuts off once the robot enters
        the ring, handing control to the orbit reward.
        """
        max_r = self.cfg["rewards"].get("orbit_max_radius", 0.6)
        ball_to_robot = self.base_pos[:, 0:2] - self.ball_pos[:, 0:2]
        dist = torch.norm(ball_to_robot, dim=-1)
        to_ball_dir = -ball_to_robot / dist.clamp(min=1e-3).unsqueeze(-1)
        base_vel_xy = self.root_states[:, 0, 7:9]
        approach_speed = torch.sum(base_vel_xy * to_ball_dir, dim=-1)
        outside_ring = (dist > max_r).float()
        return torch.clamp(approach_speed, min=0.0) * outside_ring

    def _reward_face_ball(self):
        """Reward the robot's forward axis pointing toward the ball (any orbit angle)."""
        ...
        to_ball = self.ball_pos - self.base_pos
        to_ball_norm = to_ball / (torch.norm(to_ball, dim=-1, keepdim=True) + 1e-6)
        alignment = torch.sum(forward_world * to_ball_norm, dim=-1)  # range [-1, 1]
        sigma = self.cfg["rewards"].get("face_ball_sigma", 0.5)
        return torch.exp((alignment - 1.0) / sigma)


KickParameterWalk = ParameterWalkKick
